function  NIFTI_NORDIC_gfactor2(fn_magn_in,fn_phase_in,fn_out,ARG,pathw,kzz)
% fMRI
...
QQ.ARG=ARG;
%kz2 = [5 5 5];
% [Md_mppca,Sigma_mppca] = MPdenoising(KSP2(:,:,16:40,:),[],kz2,'full');

for n1=1:size(QQ.KSP_processed,2)
    % fprintf( [num2str(n1) ' '])
    [KSP_recon,~,KSP_weight,NOISE,Component_threshold,energy_removed,SNR_weight]=sub_LLR_Processing(KSP_recon,real(KSP2),ARG,n1,QQ,master_fast,KSP_weight,NOISE,Component_threshold,energy_removed,SNR_weight)   ;    % save all files
    % fprintf( [num2str(n1) ' '])
...
for n1=1:size(QQ.KSP_processed,2)
    % fprintf( [num2str(n1) ' '])
    [KSP_recon,~,KSP_weight,NOISE,Component_threshold,energy_removed,SNR_weight]=sub_LLR_Processing(KSP_recon,imag(KSP2),ARG,n1,QQ,master_fast,KSP_weight,NOISE,Component_threshold,energy_removed,SNR_weight)   ;    % save all files
    % fprintf( [num2str(n1) ' '])
    % if mod(n1,20)==0; disp(' ');end
end
KSP_recon=KSP_recon./repmat((KSP_weight),[1 1 1 size(KSP2,4)]);
...
save([pathw,num2str(kzz),'3dgfactor_realimag.mat'],'gfactorreal','gfactorimag','gfactorboth','KSP_recon','ARG','-v7.3')
