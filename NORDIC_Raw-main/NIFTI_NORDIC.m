function  NIFTI_NORDIC(fn_magn_in,fn_phase_in,fn_out,ARG,pathw)
% fMRI
...
addpath(genpath('/vols/Data/wwu/wenchuan/dager/denoise/Denoising-master/Denoising-master'))
...
%[Md_mppca,Sigma_mppca] = MPdenoising(KSP2,[],5,'full');
...
% 
% ghgh=[2 0.5 0.8 1 1.2 1.6 24];
% for gh=1:1
% % for slice=90:90
% % noisy_image(:,:,1:9)=real(squeeze(KSP2(41:end-40,26:end-25,slice,2:10)));
% % noisy_image(:,:,10:18)=real(squeeze(KSP2(41:end-40,26:end-25,slice,12:20)));
% % noisy_image(:,:,19:27)=real(squeeze(KSP2(41:end-40,26:end-25,slice,22:30)));
% % noisy_image(:,:,28:36)=real(squeeze(KSP2(41:end-40,26:end-25,slice,32:40)));
% % noisy_image(:,:,37:45)=real(squeeze(KSP2(41:end-40,26:end-25,slice,42:50)));
% % noisy_image(:,:,46:47)=real(squeeze(KSP2(41:end-40,26:end-25,slice,52:53)));
% % 
% % 
% % noisy_image(:,:,48:56)=imag(squeeze(KSP2(41:end-40,26:end-25,slice,2:10)));
% % noisy_image(:,:,57:65)=imag(squeeze(KSP2(41:end-40,26:end-25,slice,12:20)));
% % noisy_image(:,:,66:74)=imag(squeeze(KSP2(41:end-40,26:end-25,slice,22:30)));
% % noisy_image(:,:,75:83)=imag(squeeze(KSP2(41:end-40,26:end-25,slice,32:40)));
% % noisy_image(:,:,84:92)=imag(squeeze(KSP2(41:end-40,26:end-25,slice,42:50)));
% % noisy_image(:,:,93:94)=imag(squeeze(KSP2(41:end-40,26:end-25,slice,52:53)));
% 
% noisy_image(:,:,1:9)=abs(squeeze(KSP2(41:end-40,26:end-25,slice,2:10)));
% noisy_image(:,:,10:18)=abs(squeeze(KSP2(41:end-40,26:end-25,slice,12:20)));
% noisy_image(:,:,19:27)=abs(squeeze(KSP2(41:end-40,26:end-25,slice,22:30)));
% noisy_image(:,:,28:36)=abs(squeeze(KSP2(41:end-40,26:end-25,slice,32:40)));
% noisy_image(:,:,37:45)=abs(squeeze(KSP2(41:end-40,26:end-25,slice,42:50)));
% noisy_image(:,:,46:47)=abs(squeeze(KSP2(41:end-40,26:end-25,slice,52:53)));
% 
% 
% kernel=8;
% dir=47;
...
% iter=30;
% 
% noisy_image=noisy_image/max(max(max(noisy_image)));
% 
% denoise_image=zeros(size(noisy_image));
% mask=zeros(size(noisy_image));
% for ifg=1:step:size(noisy_image,1)-kernel+1
% for jfg=1:step:size(noisy_image,2)-kernel+1
% %%extract patch
% noisy_patch=noisy_image(ifg:ifg+kernel-1,jfg:jfg+kernel-1,:);
% noisy_patch=reshape(noisy_patch,[kernel*kernel,dir]);
% %%kernel function
% N=kernel*kernel;
% noisy_kernel=zeros(kernel*kernel,kernel*kernel);
% % [u,sigma,v]=svd(noisy_patch);
% % tep=diag(sigma+eps);
% % nt=tep<2*median(diag(sigma+eps));
% % sma=tep(nt);
% % sig=sqrt(median(sma));
% % sigma=sig*sig*ghgh(gh)+eps;
% 
% %noisy_kernel=zeros(kernel*kernel*kernel,kernel*kernel*kernel);
% M=dir;
% centering=0;
%     R = min(M, N);
...
%    end
%    
% sig=sqrt(sigmasq_1(t));
% sigma=sig*sig*ghgh(gh)+eps;
% 
%    
% for ii=1:N
% for jj=1:N
% noisy_kernel(ii,jj)=exp(-sum(sum((noisy_patch(ii,:)-noisy_patch(jj,:)).^2))/2/sigma);
% end
% end
% %%kernel pca
% H=eye(N)-1/N*ones(N,1)*ones(1,N);
% [alpha,D]=eig(H*noisy_kernel*H);
...
% centering=0;
%  R=N;
%     scaling = (max(N, N) - (0:R-centering-1)) / N;
%     scaling = scaling(:);
%         csum = cumsum(vals(R-centering:-1:1)); 
%         cmean = csum(R-centering:-1:1)./(R-centering:-1:1)'; 
%         sigmasq_1 = cmean./scaling;
...
% %    
% % thth=mean(diag(D));
% % th=max(sum(sum(D>4*thth)),1);
% alpha_new=alpha(:,ind(1:th));
% lambda=diag(D(ind(1:th),ind(1:th)))';
% lambda=1./sqrt(lambda);
% %%project back
% denoised_patch=[];
% M=zeros(size(noisy_kernel));
% for ii=1:th
% M=M+1./dd(ii)*alpha_new(:,ii)*alpha_new(:,ii)';
% end
% for d=1:N
% noisy_patchinput=repmat(noisy_patch(d,:),[N,1]);
% temp1=exp(-sum((noisy_patchinput-noisy_patch).^2,2)/2/sigma);
% temp1=temp1/N;
% temp2=sum(noisy_kernel,2)/N;
% temp3=sum(sum(noisy_kernel,2),1)/N/N;
% temp3=repmat(temp3,[N,1]);
% temp=alpha_new(:,:).*repmat(exp(-sum((noisy_patchinput-noisy_patch).^2,2)/2/sigma)-temp1-temp2+temp3,[1 th]);
% newtemp= sum(temp,1);
% newtemp=newtemp.*lambda;
% newtemp=repmat(newtemp,[N,1]);
% gamma=sum( newtemp.*alpha_new(:,:),2)+1/N*(1-ones(1,N)*sum( newtemp.*alpha_new(:,:),2));
% prev=noisy_patch(d,:);
% %  noisy_patchinput2=repmat(prev,[N,1]);
% k_x=noisy_kernel(:,d);
% k_x2=repmat(k_x,[1 N]);
% KKK=1/N*noisy_kernel*ones(N,1);
% KKK2=repmat(KKK,[1 N]);
% % change k_fun
% %  noisy_patchinput3=repmat(noisy_patch(iii,:),[N,1]);
% % k_fun=exp(-sum((noisy_patchinput2-noisy_patch).^2,2)/2/sigma);
% k_xi=noisy_kernel(:,:);
% dis=(k_x2+KKK2-2*k_xi)'*H'*M*H*(k_x-KKK)+1/N/N*ones(1,N)*noisy_kernel*ones(N,1)-2/N*(ones(1,N)*k_xi)'+1;
% k_fun=0.5*(2-dis);
% down=k_fun.*gamma;
% up=repmat(down,[1,dir]).*noisy_patch(:,:);
% new=sum(up,1)./sum(down);
% denoised_patch(d,:)=new;
% end
% denoised_patch=reshape(denoised_patch,[kernel,kernel,dir]);
% 
% %  
% %       denoise_image(i:i+kernel-1,j:j+kernel-1,:)=  denoise_image(i:i+kernel-1,j:j+kernel-1,:)+denoised_patch;
% %        mask(i:i+kernel-1,j:j+kernel-1,:)=mask(i:i+kernel-1,j:j+kernel-1,:)+1;
% 
% % 
% 
% denoise_image(ifg+2:ifg+kernel-1-2,jfg+2:jfg+kernel-1-2,:)=  denoise_image(ifg+2:ifg+kernel-1-2,jfg+2:jfg+kernel-1-2,:)+denoised_patch(3:end-2,3:end-2,:);
% 
% mask(ifg+2:ifg+kernel-1-2,jfg+2:jfg+kernel-1-2,:)=mask(ifg+2:ifg+kernel-1-2,jfg+2:jfg+kernel-1-2,:)+1;
% end
% end
...
% 
% 
% denoise_image2=denoise_image(:,:,1:47)+1i*denoise_image(:,:,48:94);
% 
% temp=noisy_image(:,:,1:27)+1i*noisy_image(:,:,28:54);

save([pathw,'inter9pa.mat'],'KSP2','gfactor','ARG','meanphase','DD_phase','-v7.3')
...
% for yu=1:27
% denoise_image2(:,:,yu)=denoise_image2(:,:,yu).*gfactor(41:end-40,26:end-25,90);
% end
for n=1:size(IMG2,4);
