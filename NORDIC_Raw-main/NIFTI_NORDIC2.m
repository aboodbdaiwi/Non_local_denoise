function  NIFTI_NORDIC2(fn_magn_in,fn_phase_in,fn_out,ARG)
% fMRI
...
noisy_image=abs(squeeze(KSP2(41:end-40,26:end-25,slice,1:53)));
...
denoised_patch=reshape(denoised_patch,[kernel,kernel,dir]);
      ...
      denoise_image(ifg:ifg+kernel-1,jfg:jfg+kernel-1,:)=  denoise_image(ifg:ifg+kernel-1,jfg:jfg+kernel-1,:)+denoised_patch;
       mask(ifg:ifg+kernel-1,jfg:jfg+kernel-1,:)=mask(ifg:ifg+kernel-1,jfg:jfg+kernel-1,:)+1;
...
temprt=denoise_image;
%denoise_image2=denoise_image(:,:,1:47)+1i*temprt(:,:,1:47);

% temp=noisy_image(:,:,1:27)+1i*noisy_image(:,:,28:54);

KSP22=abs(KSP2);
for n1=1:size(QQ.KSP_processed,2)
    [KSP_recon,~,KSP_weight,NOISE,Component_threshold,energy_removed,SNR_weight]=sub_LLR_Processing(KSP_recon,KSP22,ARG,n1,QQ,master_fast,KSP_weight,NOISE,Component_threshold,energy_removed,SNR_weight)   ;    % save all files
end
